...
// Assuming the user is logged in via Firebase Auth to get the UID for 'My Donations'
...
const Fundraising = () => {
    // 💡 LOCAL CONSTANT: Donation Tiers (Fixing the scope issue)
    const donationTiers = [
    ...
    // 💡 LIVE STATE (Initialised with fallback data)
    ...
    // Endpoint must match the Python route
    ...
    useEffect(() => {
        const fetchFundraisingData = async () => {
            ...
            // Get current user's UID for fetching personalized donations
            // Note: In a production app, use onAuthStateChanged to ensure the user is loaded.
            ...
            try {
                ...
                if (response.ok && result.data) {
                    ...
                    // ✅ FIX: Use simple checks and optional chaining to safely update state
                    const liveCampaigns: Campaign[] = data.campaigns || [];
                    const liveDonors: Donor[] = data.recentDonors || [];
                    const liveMyDonations: Donation[] = data.myDonations || [];
                    ...
                    console.error("API returned error or empty shell. Using dummy data.");
                    // Fallback to DUMMY data (already in state)
                }
            } catch (error) {
                console.error("Network error fetching fundraising data:", error);
                // State remains initialized with DUMMY data
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const filteredCampaigns = campaigns.filter(campaign => {
        ...
        // Null safety added by using || '' in the filter logic
